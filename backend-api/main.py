...
async def upload_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        # # 构造多模态内容结构
        # prompt = """
        # 请阅读截图中的主体内容，判断其是否为一道题目。如果是题目，请根据以下规则进行回答：
        
        # 1. 如果是 **算法题**，请先判断考察的知识点，然后用 C++ 给出完整代码解法，并在代码前简要说明解题思路。
        # 2. 如果是 **选择题**，请直接给出答案选项（如 A/B/C/D或填空），然后进行解释说明。
        # 3. 如果不是题目，也请简要描述截图内容的主题或意图。
        
        # 格式要求：
        # 1. 请不要将整体答案包裹在 ```markdown ...``` 中，请直接输出 Markdown 内容（如标题、列表、代码块等），并在需要时单独使用语言标识。
        # 2. 请以 Markdown 格式返回，**答案优先，解释其后**，并使用代码块包裹代码（如 ```cpp ```）。
        
        # 注意：你是一个专业的解题 AI 助手，回答要准确、简洁、清晰。
        # """
        prompt = r"""
